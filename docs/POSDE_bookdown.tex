...
\date{2021 - 2026: ~Last compiled: 2026-06-18}
...
\subsection*{Acknowledgemetns}\label{acknowledgemetns}
\addcontentsline{toc}{subsection}{Acknowledgemetns}

The author would like to thank Prof.~Thomas Sauter and Prof.~Yannick Baraud for the opportunity to teach this course at the University of Luxembourg. I would also like to acknowledge the support from my former managers, Prof.~Reinhard Schneider and Dr.~Venkata Satagopam at the Luxembourg Centre for Systems Biomedicine. Similarly, I am grateful to my current supervisors, Dr.~Maximilian Fünfgeld and Dr.~Michaël Schnell at the Luxembourg Institute of Health, for supporting my teaching work. Special thanks go to Dr.~Ilhaam Isaacs from the Oxford Department for Continuing Education for her valuable insights into the philosophy of science. I would like to thank Dr.~Beatriz Garcia from the Luxembourg Centre for Systems Biomedicine for her helpful and honest feedback on the course content. Finally, I thank the students for their attention and useful feedback that helps improving the course.

Students who contributed to make this book better: Baraa Mohandes; Hamed Vaheb; Max Sinner; Clara Duchossois; Seyedehzahra Hashemi; Hesam Korki; Kliment Berbatov.
...
{\centering \includegraphics[width=0.22\linewidth]{Figures/Arthur_Stanley_Eddington} 

}

\caption{English astrophysicist Sir Arthur Stanley Eddington (1882–1944)}\label{fig:eddington-photo}
\end{figure}
...
However, during the late 1950s and early 1960s, whether or not smoking caused lung cancer was not clear. The subject of the debate wasn't tobacco or cancer but rather the word \emph{caused} as one of the most important arguments against the smoking-cancer hypothesis was the possible existence of confounding factors that may cause lung cancer and nicotine dependency. Many smokers live long lives without getting lung cancer while others develop cancer without ever smoking. Plotting the rates of lung cancer and tobacco consumption makes the connection impossible to miss (See Figure \ref{fig:smoke-ts}). However, time-series data are poor evidence for causality. Researchers already knew about Randomised Control Trials (RCT) though its use was unethical in this case because you cannot assign a group of participants to smoke.
...
We won't focus on how this story ends here but \textbf{it is important to notice how methods chosen based on scientific reasons are sometimes tough to implement and often need to fight against outcome-oriented studies} such as those sponsored by leading tobacco companies.
...
The \textbf{study power} is defined as the ability of a study to detect an effect or association if one truly exists in the population. Clinical studies are conducted on a subset of the patient population because measuring a characteristic in the entire population is not feasible. Whenever a statistical inference is made from a sample, it is subject to some error. Researchers attempt to reduce systematic errors through proper design, leaving only random errors to contend with. There are two types of random errors to consider when making inferences about the studied population: \textbf{type I and type II errors}.

To make a statistical inference, two hypotheses must be set: the \textbf{null hypothesis} (there is no difference) and the \textbf{alternate hypothesis} (there is a difference). The probability of reaching a statistically significant result when in truth there is no difference---or of rejecting the null hypothesis when it should have been accepted---is denoted as \(\alpha\), or the \textbf{probability of type I error}. It is analogous to the false positive result of a clinical test. The probability of not detecting a minimum clinically important difference when in truth there is a difference---or of accepting the null hypothesis when it should have been rejected---is denoted as \(\beta\), or the \textbf{probability of type II error}. It is analogous to the false negative result of a clinical test. Ideally, researchers choose the values of \(\alpha\) and \(\beta\) before gathering data so that their choices cannot be influenced by study results. The typical value of \(\alpha\) is set at 0.05, and the significance level (\(p\) value) determined from the data is compared with \(\alpha\) to decide on statistical significance. The typical value of \(\beta\) is set at 0.2. Study power, its complement, equals \(1-\beta\) and is commonly reported as a percentage. --- Adapted from \citep{cadeddu2008users}
...
\item
  The hypothesis should be positive. That is, the hypothesis should make a positive statement about the existence of a relationship or effect, rather than a statement that a relationship or effect does not exist.
\item
  The hypothesis should have some generality (e.g.~``things of certain type\ldots{}'') or be about some non-directly observable property of a particular.
\end{itemize}
...
An additional problem to the compatibility issue, is that many hypotheses are compatible with any given observation. This is called the problem of underdetermination.
...
It is also important to notice how the problem of underdetermination relates to the popular \textbf{No Free Lunch Theorem} (NFL) which is very relevant in the Machine Learning community \citep{dotan2020theory}. For more on the NFL read \citep{domingos2015master}.
...
See Figure \ref{fig:diagram-experiments} for an schema depicting the conceptualisation of natural and quasi-experiments. Some authors differentiate between natural experiments and \emph{quasi-experiments}. In a quasi-experiment, the criterion for group assignment of the study units (e.g.~study participants) is selected by the researchers, whereas, in a natural experiment, the assignment occurs \emph{naturally}, without the intervention of the researchers. For instance, a city implements a new traffic calming policy in one district but not another, and researchers compare accident rates between the two areas. In this case the districts were not randomly assigned, but the policy change creates a natural treatment and control group.
...
\item
  Interference: Interference occurs when experimental units alter each other's outcomes. This generates a bias that prevents the proper estimation of causal effects by the reseachers.
\item
...
\begin{verbatim}
## Estimated Effect (revised model): 10.095828768814442
\end{verbatim}
...
\begin{quote}
``If we assume that a given \textbf{covariate} influences both the hypothesised risk factor and the outcome (a confounder), it is appropriate to condition on that covariate to remove bias induced by the confounding structure. However, if the covariate is a consequence of either or both the exposure and the outcome (a collider), rather than a common cause (a confounder), then conditioning on the covariate can induce bias rather than reduce it. That is, \textbf{collider bias can also be introduced when making statistical adjustments for variables that lie on the causal pathway between risk factor and outcome}. A priori knowledge of the underlying causal structure of variables and whether they function as a common cause or common consequence of risk factor and outcome in the data generating process can be hard to infer. Therefore, it is appropriate to treat collider bias with a similar level of caution to confounding bias.'' --- \citep{griffith2020collider}
\end{quote}
...
Still, the obtained efficacy is lower than what we would expect. There are other factors that contribute to this confusion, including: age disparity in vaccinations, old people are more likely to be hospitalized than young people, etc.
...
This kind of data is common in schools, hospitals, etc. Collection of children's data generally entails writing a privacy notice simple enough to be understood by a child. Some additional steps may be needed to prove a person's age. For more information see \href{https://gdpr-info.eu/art-8-gdpr/}{Art. 8 of the GDPR}.
...
\item
  \textbf{Erasure}: This right is also known as the right to be forgotten. This right has limitations. For instance, removing a data subject from all backups retrospectively. Should this be done there would not be backups any more. Therefore, a best effort must be made and in the case of backups, we should keep a list of removed data subjects to erase their records upon recovery should we need to restore a backup.
\item
...
\chapter{Course Exercises}\label{course-exercises}

\section{The issues of data synthesis}\label{the-issues-of-data-synthesis}

\subsection{Greenland's size}\label{greenlands-size}



\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{Figures/mercator} 

}

\caption{The left map shows an orthographic projection, emulating a globe. The right map is a Mercator projection, which exaggerates the sizes of landmasses, including Greenland. Alyson Hurt / NPR}\label{fig:greenland-map}
\end{figure}
...
\textbf{How to approach the answers:}

This exercise is designed to help you reason about \textbf{distortion introduced by data synthesis and representation}, rather than errors in measurement or data collection. The goal is not to test geographical knowledge, but to see how \emph{true data} can still lead to \emph{false beliefs} once transformed or visualised.
...
Typical candidates include averages hiding variability, misleading axes in charts, heatmaps without normalization, or global indicators masking regional disparities. The key is to show that \emph{nothing is fabricated}, but interpretation is nonetheless skewed.

For the second question, clearly separate \textbf{data collection} from \textbf{data representation}. Ask yourself:
...
Your explanation should make clear that the problem arises \emph{after} data collection, during transformation, aggregation, or visualisation choices. This reinforces the idea that epistemic failure often occurs downstream from measurement.

The third question invites you to connect epistemic distortion to \textbf{social and ethical consequences}. When thinking about biased narratives or echo chambers, consider how repeated exposure to the same distorted representation can:
...
\item
  explain when representations cease to be descriptive tools and become \emph{world-shaping constructs}.
\end{itemize}

This question tests your ability to integrate epistemology, data practice, and ethics, rather than recall factual content.

Overall, approach this exercise by keeping in mind that \textbf{data visualisations are not neutral windows onto reality}. They are models and syntheses that highlight some aspects while obscuring others, and this selective emphasis is precisely where both insight and distortion originate.

\section{Learning from History}\label{learning-from-history}
...
\subsection{Al-Razi and Hospital Location}\label{al-razi-and-hospital-location}
...
\tightlist
\item
  Find the \textbf{epistemic role of a proxy} and distinguish it from direct measurement.
\item
  Think about \textbf{what is being inferred vs.~what is observed}.
\item
...
\item
  For the analogy, map \emph{proxy to target} in modern data science (e.g., indirect indicators).
\end{itemize}

\subsection{Ibn Sina and Medical Evidence}\label{ibn-sina-and-medical-evidence}
...
In the 11th century, \textbf{Ibn Sina (Avicenna)} reflected critically on medical practice. He observed that physicians often inferred the effectiveness of treatments after witnessing a few apparent recoveries, without considering alternative explanations.
...
\tightlist
\item
  Focus on \textbf{post hoc reasoning} and confounding.
\item
  Identify the difference between \textbf{correlation, coincidence, and causation}.
\item
...
\subsection{Florence Nightingale and Military Mortality}\label{florence-nightingale-and-military-mortality}



\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{Figures/learning_from_history_nightingale} 

}

\caption{Florence Nightingale's 1858 `Rose Diagram'. It uses color-coded wedges to show that more soldiers died from preventable infections (blue) than from battlefield wounds (red). By comparing mortality before and after sanitary reforms, she visually proved that basic hygiene and ventilation drastically reduced death rates, transforming military healthcare forever.}\label{fig:florence-pic}
\end{figure}

During the Crimean War in the 1850s, \textbf{Florence Nightingale} confronted widespread assumptions about military deaths. High mortality among soldiers was often attributed directly to battle injuries, reinforcing the belief that little could be done beyond improving combat medicine.

Nightingale collected detailed hospital records and noticed that most deaths occurred away from the battlefield. Rather than relying on total death counts, she reorganised the data by \emph{cause of death} and \emph{time period}. She then compared mortality patterns before and after sanitation reforms, using visualisations to communicate her findings.
...
\tightlist
\item
  Contrast \textbf{raw counts} with \textbf{structured comparisons}.
\item
...
\item
  Link ethics to \textbf{prevention capacity}, responsibility, and policy impact.
\end{itemize}

\subsection{The Collapse of Angkor}\label{the-collapse-of-angkor}



\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{Figures/learning_from_history_angkor} 

}

\caption{Facade of Angkor Wat, a drawing by Henri Mouhot (1860)}\label{fig:angkor-pic}
\end{figure}

When French explorer \textbf{Henri Mouhot} encountered Angkor in the 19th century, he interpreted its overgrown monuments as evidence of sudden civilisational collapse. This interpretation relied heavily on visible ruins as proxies for societal vitality.
...
\subsection{Goldziher: From Textual Corpora to Historical Claims}\label{goldziher-from-textual-corpora-to-historical-claims}
...
In the late nineteenth century, \textbf{Ignaz Goldziher} studied large corpora of Islamic legal and religious texts. Earlier scholars treated these texts as direct historical observations, assuming that many similar reports implied factual accuracy about early Islamic history.
...
Goldziher's insight was not that the data were useless, but that they measured \emph{something different} from what researchers often assumed. This distinction between data generation and target phenomenon is central to modern data science.
...
\tightlist
\item
  Explicitly distinguish \textbf{observed variables} from \textbf{target constructs}.
\item
  Use the idea of \textbf{generative processes} and feedback loops.
\item
...
\subsection{Wu Lien-teh and the Manchurian Plague}\label{wu-lien-teh-and-the-manchurian-plague}



\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{Figures/learning_from_history_wu} 

}

\caption{Dr Wu Lien-Teh working in his laboratory in Harbin, China, 1911}\label{fig:manchuria-pic}
\end{figure}

In the early 1910s, during a plague outbreak in Manchuria, prevailing medical theories emphasised flea-borne transmission. \textbf{Wu Lien-teh} observed rapid spread among people with close contact, including medical staff, suggesting airborne transmission.
...
\subsection{Janet Lane-Claypon and Breast Cancer Research}\label{janet-lane-claypon-and-breast-cancer-research}
...
In the early 20th century, \textbf{Janet Lane-Claypon} investigated possible causes of breast cancer. Many suspected factors were common among affected women, but this alone did not establish causation.
...
\tightlist
\item
  Emphasise \textbf{base rates} and comparison groups.
\item
...
\subsection{Terranova, the Northern Cod Collapse}\label{terranova-the-northern-cod-collapse}
...
{\centering \includegraphics[width=0.75\linewidth]{Figures/learning_from_history_cod} 

}

\caption{Collapse of Atlantic cod stocks off Newfoundland (1992): Offshore trawling since the 1950s drove catches up but biomass down, and international and national quotas failed to stop the decline.}\label{fig:code-pic}
\end{figure}
...
\subsection{Li Wenliang and Early COVID-19 Signals}\label{li-wenliang-and-early-covid-19-signals}
...
In December 2019, \textbf{Li Wenliang}, an ophthalmologist in Wuhan, noticed several patients with unusual pneumonia symptoms resembling earlier SARS cases. At that stage, the number of cases was very small, no pathogen had been identified, and no epidemiological studies were available.
...
\tightlist
\item
  Contrast \textbf{ideal evidence} with \textbf{available evidence}.
\item
