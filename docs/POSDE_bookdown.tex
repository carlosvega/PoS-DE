...
\subsection{Dr.~Arbuthnot's ``Male Years''}\label{dr.-arbuthnots-male-years}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Start with the first birth of 1628. Go back to 1627, and the act of copulation that began the pregnancy. Trace the movements of the sperm to the fertilization of the egg, Derive the chromosomal character of the sperm that won the race. Follow the process of pregnancy and birth. You have now explained the sex of the first birth of 1628. Enter it, under \texttt{M} or \texttt{F}, in a tabulation.
\item
  Repeat this for the second birth of 1628, and continue through all the births of 1628. Now add up the totals in each column of your tabulation.You have now explained why 1628 was a ``male year''.
\item
  Repeat the same procedure for 1629, 1630, and all the years up to 1709. You now have a complete causal explanation of why all these years were ``male''.
\end{enumerate}
...
--- Extracted from \citep{barker-pos}
\end{quote}

\begin{itemize}
\tightlist
\item
  \textbf{Step 1. Real World.} London birth records between 1628 and 1709 show that every one of the 82 years had more male than female births.
\item
  \textbf{Step 2. Model(s).} (1) Arbuthnot's hypothesis: The consistent excess of male births is evidence of divine providence, God's design ensures population balance. (2) Fischer later hypothesis: Natural selection favors an equilibrium sex ratio of 1:1 at sexual maturity. Since boys have higher mortality, more males must be born.
\item
  \textbf{Step 3. Prediction.} Arbuthnot's model: We should always observe more male births, reflecting divine design. If Fisher's model is correct, then large populations should show slightly more male births than female births every year, with fluctuations due to chance but always biased toward males.
\item
  \textbf{Step 4. Data.} The London records show 82 consecutive ``male years.'' Modern demographic data confirm a consistent pattern: the human birth ratio is indeed slightly male-biased across large populations.
\item
  \textbf{Step 5. Negative Evidence?} Not really. The observed data fit both models: both predict male-skewed births. There is no direct contradiction with either hypothesis.
\item
  \textbf{Step 6. Positive Evidence?} Unclear. Fischer model explains why such a pattern is expected, it ties the bias to reproductive success and evolutionary stability. Divine providence hypothesis adds no predictive or mechanistic insight.
\end{itemize}
