...
\date{2021 - 2025: ~Last compiled: 2025-07-24}
...
\includegraphics{POSDE_bookdown_files/figure-latex/unnamed-chunk-30-1.pdf}
...
The backdoor criterion is a method used in causal inference to control for confounders---variables that influence both the treatment and the outcome. By satisfying the backdoor criterion, we aim to isolate the causal effect of the treatment on the outcome by ``blocking'' other paths of influence. Suppose we have a causal structure where a variable\texttt{Z} (like prior knowledge) affects both our treatment \texttt{X} (study hours) and outcome \texttt{Y} (exam score). The backdoor criterion tells us to condition on (i.e., adjust for) variables like \texttt{Z} which lie on ``backdoor paths'' that might introduce bias in estimating the effect of \texttt{X} on \texttt{Y}. By conditioning on \texttt{Z}, we effectively ``block'' any non-causal paths from \texttt{X} to
\texttt{Y}, allowing us to observe the effect of \texttt{X} on \texttt{Y} without interference from confounding.
...
\includegraphics{POSDE_bookdown_files/figure-latex/unnamed-chunk-36-3.pdf}
...
\includegraphics{POSDE_bookdown_files/figure-latex/unnamed-chunk-45-5.pdf}
