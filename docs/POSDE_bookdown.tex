...
\makeatother
\usepackage{xcolor}
\usepackage[lmargin=4cm,rmargin=3cm,bmargin=3cm]{geometry}
\usepackage{color}
\usepackage{fancyvrb}
\newcommand{\VerbBar}{|}
...
\date{2021 - 2024: Last compiled: 2024-11-05}
...
Since October 2023 \href{https://researchportal.lih.lu/en/persons/carlos-vega}{Carlos Vega} works as a Research Engineer in Digital Health at the \textbf{Luxembourg Institute of Health} (LIH) in the IT Expertise in Health Data team of the Data Integration and Analysis Unit led by the CMIO (Chief Medical Information Officer) Dr.~Maximilian Fünfgeld.
...
Here, the test has replaced the actual treatment (study\_hours) with a ``placebo'' or random variable that should have no causal effect on the outcome (exam\_score).
...
We generate a synthetic data with different random values. As you can see parental\_involvement is biased with a 70\% probability to be 0 and 30\% of being 1. In the last two lines we fill tutoring and grade as we have synthetically made them to depend on the study hours and parental involvement. Imagine that instead of being synthetically created this data was given to you from data collection (e.g.~via surveys).
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bootstrap\_refutation }\OperatorTok{=}\NormalTok{ model.refute\_estimate(identified\_estimand, estimate, }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sensitivity\_analysis }\OperatorTok{=}\NormalTok{ model.refute\_estimate(identified\_estimand, estimate, }
